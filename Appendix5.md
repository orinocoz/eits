# Appendix 5: NIST CSF 2.0 Category Catalogue

This catalogue lists the CSF 2.0 Functions and Categories used in Appendix 3.

Source: NIST, *The NIST Cybersecurity Framework (CSF) 2.0*, CSWP 29, February 26, 2024, https://doi.org/10.6028/NIST.CSWP.29. Accessed: May 11, 2026.

| Function code | Function | Purpose |
|---|---|---|
| GV | Govern | Establish and oversee cybersecurity risk strategy, expectations, and policy. |
| ID | Identify | Understand the organization's current assets, context, and cybersecurity risks. |
| PR | Protect | Apply safeguards to reduce the likelihood and impact of cybersecurity events. |
| DE | Detect | Find and analyze possible attacks, anomalies, and compromises. |
| RS | Respond | Take action to contain and manage detected cybersecurity incidents. |
| RC | Recover | Restore affected assets and operations after a cybersecurity incident. |

| Function | Category code | Category name |
|---|---|---|
| Govern | GV.OC | Organizational Context |
| Govern | GV.RM | Risk Management Strategy |
| Govern | GV.RR | Roles, Responsibilities, and Authorities |
| Govern | GV.PO | Policy |
| Govern | GV.OV | Oversight |
| Govern | GV.SC | Cybersecurity Supply Chain Risk Management |
| Identify | ID.AM | Asset Management |
| Identify | ID.RA | Risk Assessment |
| Identify | ID.IM | Improvement |
| Protect | PR.AA | Identity Management, Authentication, and Access Control |
| Protect | PR.AT | Awareness and Training |
| Protect | PR.DS | Data Security |
| Protect | PR.PS | Platform Security |
| Protect | PR.IR | Technology Infrastructure Resilience |
| Detect | DE.CM | Continuous Monitoring |
| Detect | DE.AE | Adverse Event Analysis |
| Respond | RS.MA | Incident Management |
| Respond | RS.AN | Incident Analysis |
| Respond | RS.CO | Incident Response Reporting and Communication |
| Respond | RS.MI | Incident Mitigation |
| Recover | RC.RP | Incident Recovery Plan Execution |
| Recover | RC.CO | Incident Recovery Communication |
